...
const UpgradeCard: React.FC<I_upgrade_card> = ({
  ...
  return (
    ...
    >
      <p className=" text-[18px] lg:text-[16px] italic">{title}</p>
      <div className=" w-full flex ">
        <span>
          <p className=" text-4xl lg:text-[22px]">R{price}</p>
        </span>
        <span className=" flex flex-col items-start justify-center pl-2">
          <p className=" text-[16px] lg:text-[12px] ">Per month</p>
          <p className=" text-[14px] lg:text-[10px] ">discounts available</p>
        </span>
      </div>
      ...
      <div className=" w-full h-full flex flex-col items-start justify-around py-2">
        {features.map((feature, index) => (
          <div key={feature} className=" w-full h-fit py-2 flex items-center">
            {icon}
            <p className=" text-[16px] lg:text-[12px] ">{feature}</p>
          </div>
        ))}
      </div>
    </div>
